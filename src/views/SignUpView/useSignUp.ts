...
export function useSignUp(): { state: LoginState, onEmailChange: (email: string) => void, onPasswordChange: (password: string) => void, signUp: () => void } {
    const navigation = useNavigation();
    const [state, dispatch] = useReducer(reducer, initialState)
    const signUp = useCallback(async () => {
        try {
            if (state.email && state.password) {
                await Framework.getInstance().AuthenicationService.createUser(state.email, state.password);
            }
        } catch (error) {
            console.log(error);
        }
