...
export function useEditNoteView(): EditNoteViewProps {
    ...
    const onSavePress = (note: Note) => routeNote ? dispatch(updateExistingNote(routeNote?.id, note, onError)) : dispatch(addNote(note, onError));
    const extractNote = async (response: ImagePickerResponse) => {
        if (response.assets[0].base64) {
            const text = await Framework.getInstance().NotesService.extractNote(response.assets[0].base64);
            setBodyValue(`${bodyValue} ${text}`)
        }
    }
    const onCameraPress = () => {
        launchCamera({ mediaType: "photo", includeBase64: true, maxHeight: 1000, maxWidth: 500 }, extractNote)
    }


    return { titleValue, setTitleValue, bodyValue, setBodyValue, isReminder, setIsReminder, dismiss, onSavePress, onCameraPress }
}